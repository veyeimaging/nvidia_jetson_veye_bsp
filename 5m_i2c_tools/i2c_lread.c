...
static int i2c_rd(int fd, uint8_t i2c_addr, uint32_t reg, uint32_t *values, uint32_t n)
{
	int err;
	int i = 0;
	uint8_t buf[4] = { reg >> 24, reg >> 16, reg >> 8, reg & 0xff };
    uint8_t bufout[10] = {0};
...
U32 main(int argc, char *argv[])
{
	...
	if (argc < 4)
	{
		printf("usage: %s <bus_num> <device address(8bit)> <register address(32bits)> <len default:1 max100>. sample: %s 0x0 0xA0 0x1000 0x2\n", argv[0], argv[0]);
		return -1;
	}
